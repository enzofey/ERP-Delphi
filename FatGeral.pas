  ...
  try
   ...
   if CBGeraDupl.ItemIndex = 0 then begin
    ...
    if EdtPrazo1.Text = '' then Dias:= 0 else Dias := StrToInt(EdtPrazo1.Text);
     ...
     begin
      SQL.Clear;
      SQL.Add('insert into receber (dtemissao, dtvencto, numero, entidade, fatura, valor, valor_pago, status, moeda, parcela, valor_parcela, emp_id) ');
      SQL.Add('values ');
      SQL.Add('(:dtemissao, :dtvencto, :numero, :entidade, :fatura, :valor, :valor_pago, :status, :moeda, :parcela, :valor_parcela, :emp_id)');
