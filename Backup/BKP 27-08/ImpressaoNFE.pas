...
begin
    ...
    ImpressaoNFEFormDataModule.TransmitirNotaQuery.ParamByName('nota_fatura').AsString := GridNotas.DataSource.DataSet.FieldByName('fatura').AsString;
    ImpressaoNFEFormDataModule.TransmitirNotaQuery.Open;

    Aliq_Cred_pres := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('natureza_AliqCredPres').AsFloat;
    vNF := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valortotal').AsFloat;

    // CHAVE_NFE

    cUF := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('estado_codigo').AsString;
    AAMM := FormatDateTime('yyMM', Now);
    CNPJEmi := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('Emp_CNPJ').AsString;
    serie := FormatFloat('000', ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_serie').AsInteger);
    fatura := FormatFloat('000000000', ImpressaoNFEFormDataModule.TransmitirNotaQuery.ParamByName('nota_fatura').AsInteger);
  ...
  try
    ...
    idenode.AddChild('natOp').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('natureza_descricao').AsString;

    idenode.AddChild('mod').Text := '55';

    idenode.AddChild('serie').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_serie').AsString;

    idenode.AddChild('nNF').Text := IntToStr(StrToInt(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_fatura').AsString));

    idenode.AddChild('dhEmi').Text := FormatDateTime('yyyy"-"mm"-"dd"T"hh":"nn":"ss"-03:00"', ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_dtemissao').AsDateTime);

    idenode.AddChild('dhSaiEnt').Text := FormatDateTime('yyyy"-"mm"-"dd"T"hh":"nn":"ss"-03:00"', ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_dtsaida').AsDateTime);

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('natureza_tipo').AsString = 'S' then idenode.AddChild('tpNF').Text := '1'
    else idenode.AddChild('tpNF').Text := '0';

    case ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_natureza').AsString [1] of
    '1', '5': idenode.AddChild('idDest').Text := '1';
    '2', '6': idenode.AddChild('idDest').Text := '2';
    '3', '7': idenode.AddChild('idDest').Text := '3';
    end;

    idenode.AddChild('cMunFG').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('cidade_codigo').AsString;
    ...
    case ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_consumidorfinal').AsString [1] of
    'N': idenode.AddChild('indFinal').Text := '0';
    'S': idenode.AddChild('indFinal').Text := '1';
    end;

    idenode.AddChild('indPres').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_presenca').AsString;
    ...
    EnderEmitNode.AddChild('cMun').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('cidade_codigo').AsString;
    ...
    EnderEmitNode.AddChild('cPais').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('pais_codigo').AsString;
    EnderEmitNode.AddChild('xPais').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('pais_nome').AsString;
    ...
    if Length(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_CPF').AsString) = 14 then begin
    destnode.AddChild('CNPJ').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_CPF').AsString;
    end;
    if Length(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_CPF').AsString) = 11 then begin
    destnode.AddChild('CPF').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_CPF').AsString;
    end;
    ...
    enderdestnode.AddChild('cMun').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('cidade_codigo').AsString;
    ...
    enderdestnode.AddChild('cPais').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('pais_codigo').AsString;
    enderdestnode.AddChild('xPais').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('pais_nome').AsString;
    enderdestnode.AddChild('fone').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ent_telefone').AsString;

    destnode.AddChild('indIEDest').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('Ent_IndIE').AsString;

    if (ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('ENT_IE').AsString) <> '' then
    ...
    prodnode.AddChild('cProd').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_codigo').AsString;
    prodnode.AddChild('cEAN').Text := 'SEM GTIN';
    prodnode.AddChild('xProd').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_descricao').AsString;
    prodnode.AddChild('NCM').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_ncm').AsString;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_cBenef').AsString <> '' then
    prodnode.AddChild('cBenef').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_cBenef').AsString;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_cCredPres').AsString <> '' then
    gCredNode := prodnode.AddChild('gCred');
    gCredNode.AddChild('cCredPresumido').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_cCredPres').AsString;
    ...
    prodnode.AddChild('CFOP').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_natureza').AsString;
    prodnode.AddChild('uCom').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_unidade').AsString;
    prodnode.AddChild('qCom').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_qtde').AsFloat);
    prodnode.AddChild('vUnCom').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_precouni').AsFloat);
    prodnode.AddChild('vProd').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valor').AsFloat);
    prodnode.AddChild('cEANTrib').Text := 'SEM GTIN';
    prodnode.AddChild('uTrib').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_unidade').AsString;
    prodnode.AddChild('qTrib').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_qtde').AsFloat);
    prodnode.AddChild('vUnTrib').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_precouni').AsFloat);
    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_brinde').AsString = 'S' then prodnode.AddChild('indTot').Text := '0'
    ...
    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '00' then begin
    ICMS00Node := ICMSNode.AddChild('ICMS00');
    ICMS00Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS00Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    ICMS00Node.AddChild('modBC').Text := '3';
    ICMS00Node.AddChild('vBC').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valbaseicms').AsString;
    ICMS00Node.AddChild('pICMS').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_aliqicms').AsString;
    ICMS00Node.AddChild('vICMS').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valicms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '10' then begin
    ICMS40Node := ICMSNode.AddChild('ICMS10');
    ICMS40Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS40Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '20' then begin
    ICMS00Node := ICMSNode.AddChild('ICMS20');
    ICMS00Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS00Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '30' then begin
    ICMS40Node := ICMSNode.AddChild('ICMS30');
    ICMS40Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS40Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '40' then begin
    ICMS00Node := ICMSNode.AddChild('ICMS40');
    ICMS00Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS00Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '50' then begin
    ICMS40Node := ICMSNode.AddChild('ICMS50');
    ICMS40Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS40Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '60' then begin
    ICMS00Node := ICMSNode.AddChild('ICMS60');
    ICMS00Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS00Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '70' then begin
    ICMS40Node := ICMSNode.AddChild('ICMS70');
    ICMS40Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS40Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '80' then begin
    ICMS40Node := ICMSNode.AddChild('ICMS80');
    ICMS40Node.AddChild('orig').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('produto_origem').AsString;
    ICMS40Node.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString;
    end;

    if ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_csticms').AsString = '90' then begin
    ...
    IPINode.AddChild('cEnq').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_ENQIPI').AsString;

    IPINTNode := IPINode.AddChild('IPINT');
    IPINTNode.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_CSTIPI').AsString;

    PISNode := impostonode.AddChild('PIS');
    PISAliqNode := PISNode.AddChild('PISAliq');
    PisALiqNode.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_CSTPIS').AsString;
    PisALiqNode.AddChild('vBC').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valbasepis').AsFloat);
    PisALiqNode.AddChild('pPIS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_AliqPIS').AsFloat);
    PisALiqNode.AddChild('vPIS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valpis').AsFloat);

    COFINSNode := impostonode.AddChild('COFINS');
    COFINSAliqNode := COFINSNode.AddChild('COFINSAliq');
    COFINSALiqNode.AddChild('CST').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_CSTCOFINS').AsString;
    COFINSALiqNode.AddChild('vBC').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valbaseCOFINS').AsFloat);
    COFINSALiqNode.AddChild('pCOFINS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_AliqCOFINS').AsFloat);
    COFINSALiqNode.AddChild('vCOFINS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('notaiten_valCOFINS').AsFloat);

    TotalNode := InfNFENode.AddChild('total');
    ICMSTotNode := TotalNode.AddChild('ICMSTot');
    ICMSTotNode.AddChild('vBC').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valorbaseicms').AsFloat);
    ICMSTotNode.AddChild('vICMS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valoricms').AsFloat);
    ...
    ICMSTotNode.AddChild('vProd').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valorprodutos').AsFloat);
    ICMSTotNode.AddChild('vFrete').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valorfrete').AsFloat);
    ICMSTotNode.AddChild('vSeg').Text := '0.00';
    ICMSTotNode.AddChild('vDesc').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valordesconto').AsFloat);
    ICMSTotNode.AddChild('vII').Text :=  '0.00';
    ICMSTotNode.AddChild('vIPI').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valoripi').AsFloat);
    ICMSTotNode.AddChild('vIPIDevol').Text :=  '0.00';
    ICMSTotNode.AddChild('vPIS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valorpis').AsFloat);
    ICMSTotNode.AddChild('vCOFINS').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valorcofins').AsFloat);
    ICMSTotNode.AddChild('vOutro').Text := '0.00';
    ICMSTotNode.AddChild('vNF').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_valortotal').AsFloat);

    transpnode := infnfenode.AddChild('transp');
    transpnode.AddChild('modFrete').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_modfrete').AsString;

    volnode := transpnode.AddChild('vol');
    volnode.AddChild('qVol').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_volumes').AsString;
    volnode.AddChild('esp').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_especie').AsString;
    volnode.AddChild('pesoL').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_pesol').AsString;
    volnode.AddChild('pesoB').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_pesob').AsString;

    pagnode := infnfenode.AddChild('pag');
    detpagnode := pagnode.AddChild('detPag');

    detpagnode.AddChild('tPag').Text := Format('%.2d', [StrToInt(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('receber_moeda').AsString)]);
    detpagnode.AddChild('vPag').Text := FormataValorNFe(ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('receber_valorparcela').AsFloat);
    infAdicnode := infnfenode.AddChild('infAdic');
    infadicnode.AddChild('infCpl').Text := ImpressaoNFEFormDataModule.TransmitirNotaQuery.FieldByName('nota_dadosadicionais').AsString;
