...
begin
 ...
 ShowMessage('Código do IBGE não pode ser vazio!');
  ...
  CadPaisDataModule.UpdateQuery.SQl.Clear;
  CadPaisDataModule.UpdateQuery.SQL.Text :=
  'update cadpais set pais = :pais, ativo = :ativo, codigo = :codigoibge where sigla = :sigla';
  CadPaisDataModule.UpdateQuery.ParamByName('sigla').AsString := sigla;
  CadPaisDataModule.UpdateQuery.ParamByName('pais').AsString := pais;
  CadPaisDataModule.UpdateQuery.ParamByName('ativo').AsString := ativo;
  CadPaisDataModule.UpdateQuery.ParamByName('codigoibge').AsString := codigo;
  ...
  LogModule.InserirLog.ParamByName('emp_id').AsString := EmpresaLogada;
  try
  CadPaisDataModule.UpdateQuery.ExecSQL;
  LogModule.InserirLog.ExecSQL;
...
begin
  ...
  CadPaisDataModule.SelectQuery.ParamByName('sigla').AsString := sigla;
  CadPaisDataModule.SelectQuery.Open;

   if not CadPaisDataModule.SelectQuery.IsEmpty then begin
  ...
  if CadPaisDataModule.SelectQuery.IsEmpty then begin
  showmessage('País não encontrado');
  end
  else begin

  CadPaisDataModule.DeleteQuery.SQL.Clear;
  CadPaisDataModule.DeleteQuery.SQL.Text :=
  'delete from cadpais where sigla = :sigla';
  CadPaisDataModule.DeleteQuery.ParamByName('sigla').AsString := sigla;
  ...
  LogModule.InserirLog.ParamByName('tela').AsString := 'CadPaís';
  LogModule.InserirLog.ParamByName('data').AsDatetime := Now;
  LogModule.InserirLog.ParamByName('usuario').AsString := UsuarioLogado;
  LogModule.InserirLog.ParamByName('emp_id').AsString := EmpresaLogada;
try
  CadPaisDataModule.DeleteQuery.ExecSQL;
  LogModule.InserirLog.ExecSQL;
...
var codigo, sigla, pais, ativo: string;
begin
  EdtCodigoIBGE.Text := CadPaisDataModule.ConsultarPais.FieldByName('codigo').AsString;
  EdtSigla.Text := CadPaisDataModule.ConsultarPais.FieldByName('sigla').AsString;
  EdtPais.Text   := CadPaisDataModule.ConsultarPais.FieldByName('pais').AsString;
  CBAtivo.Checked := CadPaisDataModule.ConsultarPais.FieldByName('ativo').AsString = 'S';
end;
