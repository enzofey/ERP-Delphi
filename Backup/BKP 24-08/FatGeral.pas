...
var Produto, Cor, Tamanho, Deposito, Lote, Natureza, CST_ICMS, CSOSN, CST_IPI,
    ...
    especie, volumes, pesob, pesol, presenca, modfrete, fatura: string;
...
begin
  ...
  'select (max(cast(numero as integer)) + 1) as proximo_numero from nota_fatura where emp_id = :emp_id and serie = :serie';
  ...
  if FatGeralDataModule.SelectFatura.FieldByName('proximo_numero').AsString = '1' then fatura := '000001'
  else if FatGeralDataModule.SelectFatura.FieldByName('proximo_numero').AsString = '' then fatura := '000001'
  else fatura := FatGeralDataModule.SelectFatura.FieldByName('proximo_numero').AsString;
  ...
  try
    ...
    'O usuário ' + UsuarioLogado + ' gravou a nota ' + serie + ' na empresa ' + EmpresaLogada + ' para o cliente ' + entidade;
