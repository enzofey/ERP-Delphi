...
begin
 ...
 begin
  ...
  PAtivo := FieldByName('ativo').AsString;
  PFaixa := FieldByName('faixa').AsString;
  PNCM := FieldByName('NCM').AsString;
  PUniCom := FieldByName('UniCom').AsString;
  PUniVenda := FieldByName('UniVenda').AsString;
  PBrinde := FieldByName('Brinde').AsString;
  POrigem := FieldByName('Origem').AsString;
 end;

 with CadCorDM.qryCorEstoque do
 begin
  SQL.Clear;
  SQL.Add('select cc.codigo, cc.descricao from cadcor cc');
  SQL.Add('inner join estoque es on (es.cor = cc.codigo)');
  SQL.Add('inner join cadproduto cp on (es.codigo = cp.codigo)');
  SQL.Add('where cp.codigo = :codigo');
  SQL.Add('group by cc.codigo, cc.descricao');
